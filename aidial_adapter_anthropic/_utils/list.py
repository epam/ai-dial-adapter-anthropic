...
class ListProjection(Generic[_T]):
    ...
    elems: list[tuple[_T, set[int]]] = field(default_factory=list)

    @property
    def raw_list(self) -> builtins.list[_T]:
        return [msg for msg, _ in self.elems]

    def to_original_indices(self, indices: Iterable[int]) -> set[int]:
        return {
            orig_index
            for index in indices
            for orig_index in self.elems[index][1]
        }

    def append(self, elem: _T, idx: int) -> Self:
        self.elems.append((elem, {idx}))
        return self
